namespace ginkgoOath {
    export class DrawRewardPanel extends ui.ginkgoOath.panel.DrawRewardPanelUI {
        ...
        showReward(cell: ui.ginkgoOath.render.DrawRewardItemUI, index: number) {
            ...
            if (data.suitsAward > 0) {
                cell.imgReward.skin = "ginkgoOath/" + (data.suitsAward) + ".png";
                cell.imgCount.skin = "ginkgoOath/x1.png";
            } else {
                cell.imgReward.skin = "ginkgoOath/" + (data.maleAward[0].v1) + ".png";
                cell.imgCount.skin = "ginkgoOath/x" + data.maleAward[0].v2 + ".png";
            }
            let bgStr = "ginkgoOath/bg_reward0.png";
            if (cell.imgGet.visible == false && this._model.drawCount >= this._xlsInfoArr[index].counter) {
                bgStr = "ginkgoOath/bg_reward1.png";
            }
            cell.imgBg.skin = bgStr;
        }
        getReward(e: Laya.Event, index: number) {
            if (e.type == Laya.Event.CLICK) {
                let item = e.currentTarget as ui.ginkgoOath.render.DrawRewardItemUI;
                ...
                if (item.imgGet.visible == false && this._model.drawCount >= this._xlsInfoArr[index].counter) {
                    net.sendAndWait(new pb.cs_spirit_tree_get_draw_times_reward({ index: id })).then((data: pb.sc_spirit_tree_get_draw_times_reward) => {
                        ...
                        EventManager.event("GINKGOOATH_DRAW_REWARD_BACK");
                    });
                }
                else {
                    if ([1, 3].indexOf(index) > -1) {
                        alert.showSmall("随机获得未拥有的【北冥玄幽】服装部件，若集齐该套装，则兑换为30淘乐球", {
                            callBack: null,
