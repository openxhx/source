namespace waterSong {
    export class WaterSongModule extends ui.waterSong.WaterSongModuleUI {
        /** 抽奖功能ID*/
        private _poolId: number = 17;
        /** 抽奖使用的道具ID*/
        private _costItemId: number = 9900249;
        /**套装ID */
        private _suitId: number = 2110147;
        /**集齐奖励ID */
        private _giftId: number = 1000146;
        /**充值礼包标志 */
        /**帮助说明id */
        ...
        init() {
            ...
            clientCore.Logger.sendLog('2021年9月24日活动', '【付费】水调歌头-单套抽', '打开水调歌头-夏日告白面板');
            clientCore.MedalManager.getMedal([MedalConst.WATER_SONG_OPEN]).then((msg: pb.ICommonData[]) => {
                if (msg[0].value == 0) {
                    clientCore.MedalManager.setMedal([{ id: MedalConst.WATER_SONG_OPEN, value: 1 }]);
                    alert.showSmall('亲爱的小花仙，我们已将你口袋里剩余的棉花糖以及树枝按照1：1的比例转换为告白花束，快去参加活动吧~');
                }
            })
        }
        ...
        private initView(): void {
            ...
            //格子初始化
            this._array = _.filter(xls.get(xls.rouletteDraw).getValues(), (element: xls.rouletteDraw) => { return element.type == 186 && element.period == this._poolId; });
            this._cost = _.filter(xls.get(xls.rouletteDrawCost).getValues(), (element: xls.rouletteDrawCost) => { return element.type == 186 && element.period == this._poolId });
        ...
        addEventListeners(): void {
            ...
            BC.addEvent(this, this.addBtn, Laya.Event.CLICK, this, this.buyCoin);
            EventManager.on("COIN_RERFESH", this, this.refreshNum);
        }

        private buyCoin(){
            alert.alertQuickBuy(9900249, 1, true ,new Laya.Handler(this, (items) => {
                this.refreshNum();
            }))
        }
        ...
        private updateCurrent(): void {
            ...
            if (this.boxCurrent.visible) {
                this.imgCurOff.skin = `waterSong/${this.curDiscount / 10}.png`;
            }
        }
        ...
        private playDiscountAni(discount: number) {
            this.imgNextOff.skin = `waterSong/xia_hui${discount / 10}_zhe.png`;
            // return new Promise((suc: Function) => {
        ...
        private async onDraw(): Promise<void> {
            ...
            WaterSongModel.instance.coinCost(cost);
        ...
        {
            this.coinNum.text = clientCore.ItemsInfo.getItemNum(this._costItemId).toString();
        }
        ...
        removeEventListeners(): void {
            BC.removeEvent(this);
            EventManager.off("COIN_RERFESH", this, this.refreshNum);
        }
