namespace bigCharge {
    /**
     * 直购，两件都是根据花宝等级折扣,第二件额外折扣
     * 月下夜语、迷情甜心
     */
    export class DoubleVipOffBuyPanel extends ui.bigCharge.panel.DoubleVipOffBuyPanelUI {
        private baseBuyId1: number = 2799;
        private baseBuyId2: number = 2802;
        private buyId1: number;
        private buyId2: number;
        private suit1: number = 2110463;
        private suit2: number = 2100313;
        private giftId: number[] = [1100082, 1000113];
        ...
        private setUI() {
            ...
            //赠品
            let have3 = clientCore.ItemsInfo.checkHaveItem(this.giftId[0]);
        ...
        async show() {
            ...
            clientCore.Logger.sendLog('2021年8月13日活动', '【付费】夏日终曲第七期', '打开迷情甜心面板');
        }
        ...
        private buyGoods(idx: number) {
            ...
            if (this.imgGot1.visible || this.imgGot2.visible) price -= 160;
            ...
            alert.showSmall(`是否花费${price}${clientCore.ItemsInfo.getItemName(coin)}购买所选商品?`, {
                callBack: {
                    caller: this, funArr: [() => {
                        net.sendAndWait(new pb.cs_summer_recharge_buy({ stage: 7, idxs: [configId] })).then((msg: pb.sc_summer_recharge_buy) => {
        ...
        /**领取赠品 */
        private getGift() {
            net.sendAndWait(new pb.cs_summer_recharge_get_extra_reward({ type: 10 })).then((msg: pb.sc_summer_recharge_get_extra_reward) => {
                alert.showReward(msg.items);
