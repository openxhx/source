namespace bigCharge {
    export class SinglePanicBuy extends ui.bigCharge.panel.SinglePanicBuyPanelUI {
        private readonly suitId: number = 2110464;
        ...
        private offInfoRender(item: ui.bigCharge.render.LimitInfoRenderUI) {
            let off: number = item.dataSource;
            item.imgFlag.skin = (off >= this.curOff && this.curOff > 0) ? "bigCharge/SinglePanicBuyPanel/re_mai_zhong.png" : "bigCharge/SinglePanicBuyPanel/yi_shou_wan.png"
        ...
        async show() {
            ...
            clientCore.Logger.sendLog('2021年8月20日活动', '【付费】夏日终曲第八期', '打开夏日特卖面板');
        }
        ...
        private setUI() {
            ...
            this.imgOff.skin = `bigCharge/SinglePanicBuyPanel/dang_qian${this.curOff}_zhe.png`;
            this.listOffInfo.refresh();
        }

        /**检查开卖时间 */
        private checkSell(): number {
            let endT: number = util.TimeUtil.formatTimeStrToSec("2021/8/20 18:00:00");
        ...
        private buySuit() {
            ...
            let have = clientCore.ItemsInfo.getItemNum(9900193);
            if (have < cost) {
                alert.showSmall(`所需${clientCore.ItemsInfo.getItemName(9900193)}不足,是否前往补充?`, { callBack: { funArr: [BigChargeModel.instance.coinNotEnough], caller: this } });
                return;
            }
            let off: number = this.curOff; //打开的时候 需要缓存一下折扣值 以防打开的时候折扣发生变化... 有争议
            alert.showSmall(`是否花费${cost}${clientCore.ItemsInfo.getItemName(9900193)}购买勋贵世家套装?`, {
                callBack: {
