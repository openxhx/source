namespace timeAmbulatory {
    export class LetterBuyPanel extends ui.timeAmbulatory.panel.LittleBuyPanelUI {
        ...
        public show() {
            let model = clientCore.CManager.getModel(this._sign) as TimeAmbulatoryModel;
            ...
            for (let i: number = 0; i < configs.length; i++) {
                ...
                let rewardExtra = clientCore.LocalInfo.sex == 1 ? configs[i].femaleExtra : configs[i].maleExtra;
                for (let j: number = 0; j < rewardExtra.length; j++) {
                    let indexTotal = _.findIndex(totalReward, (o) => { return o.v1 == rewardExtra[j].v1 });
                    if (indexTotal >= 0) {
                        totalReward[indexTotal].v2 += rewardExtra[j].v2;
                    } else {
                        let _new = new xls.pair();
                        _new.v1 = rewardExtra[j].v1;
                        _new.v2 = rewardExtra[j].v2;
                        totalReward.push(_new);
                    }
                    if (configs[i].cost <= model.activeValue) {
                        let indexNow = _.findIndex(nowReward, (o) => { return o.v1 == rewardExtra[j].v1 });
                        if (indexNow >= 0) {
                            nowReward[indexNow].v2 += rewardExtra[j].v2;
                        } else {
                            let _new = new xls.pair();
                            _new.v1 = rewardExtra[j].v1;
                            _new.v2 = rewardExtra[j].v2;
        ...
        private buy() {
            clientCore.RechargeManager.pay(42).then((data) => {
                ...
                this.onClose();
                EventManager.event("TIME_MEDAL_ACTIVE");
            });
        }
