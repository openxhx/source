namespace springOrchard{
     ...
     * 奖励兑换
     */
    export class ExchangePanel extends ui.springOrchard.panel.ExchangePanelUI{

        private readonly ACTIVITY_ID: number = 135;
        ...
        show(sign: number): void{
            ...
            this.onTab(0);
            clientCore.UIManager.setMoneyIds([9900147,9900148]);
        ...
        private onTry(type: number): void{
            clientCore.ModuleManager.open("rewardDetail.PreviewModule", type == 1 ? 2110319 : 2110320);
        }
        private listRender(type: number,item: ui.springOrchard.item.ExchangeItemUI,index: number): void{
        ...
        private listMouse(type: number,e:Laya.Event,index: number): void{
            if(e.type == Laya.Event.CLICK){
                ...
                if(e.target instanceof component.HuaButton){
                    this._control?.getReward(data.id,new Laya.Handler(this,()=>{
