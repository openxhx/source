namespace plumYellow {
    export class PieceBuyPanel extends ui.plumYellow.panel.PieceBuyPanelUI {

        private buyState: number;
        private timeStamp: number;
        private suitId: number = 2110664;
        private haveTimer: Boolean = false;
        private leftTime: number = 0;
        private suitCost:number = 349;
        private pieceCost:number = 93;
        private coin: number = 9900334;
        private gift:number[] = [3500134 ] ;
        ...
        show(box:any) {
            ...
            this.suitShow.skin = `res/rechargeCloth/${this.suitId}_${clientCore.LocalInfo.sex}.png`;
            for (let i: number = 0; i < 3; i++) {
                this["blockbg" + i].skin = clientCore.LocalInfo.sex == 1 ? `plumYellow/PieceBuyPanel/blockfemale_${i}.png` : `plumYellow/PieceBuyPanel/blockmale_${i}.png`;
            }
            EventManager.event(CHANGE_TIME, "time_27_9");
            box.addChild(this);
            clientCore.Logger.sendLog('2022年5月27日活动', '【付费】梅子黄时', '打开琉璃碎夏面板');
        }
                    ...
                    funArr: [() => {
                        if (have < this.pieceCost) {
                            ...
                            net.sendAndWait(new pb.cs_patch_buy_reward({ index: i + 1  , type:1})).then((msg: pb.sc_patch_buy_reward) => {
                                PlumYellowModel.instance.coinCost(this.pieceCost);
                    ...
                    funArr: [() => {
                        if (have < this.suitCost) {
                            ...
                            net.sendAndWait(new pb.cs_patch_buy_reward({ index: 4  , type:1})).then((msg: pb.sc_patch_buy_reward) => {
                                PlumYellowModel.instance.coinCost(this.suitCost);
                                alert.showReward(msg.item);
                                net.sendAndWait(new pb.cs_patch_buy_info({type:1})).then((msg: pb.sc_patch_buy_info) => {
                                    this.buyState = msg.buyBit == null ? 0 : msg.buyBit;
                                    this.timeStamp = msg.time == null ? 0 : msg.time;
