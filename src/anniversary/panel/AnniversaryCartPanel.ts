namespace anniversary {
    export class AnniversaryCarPanel extends ui.anniversary.panel.AnniversaryCartPanelUI {
        ...
        private onListRender(cell: ui.anniversary.render.AnniversaryCartRenderUI, idx: number) {
        ...
        private setPrice() {
            ...
            if (this.selectId.length >= 6) {
                price -= 100;
            } else if (this.selectId.length >= 3) {
                price -= 30;
            }
        ...
        private async onBuy() {
            ...
            let has: number = clientCore.ItemsInfo.getItemNum(1511008);
            let diff: number = Number(this.labPrice.text) - has;
            if (diff > 0) {
                alert.alertQuickBuy(1511008, diff, true);
                return;
            }
        ...
        private buy() {
            return net.sendAndWait(new pb.cs_morning_trail_buy_clothes({ exchangeId: this.selectId })).then((data: pb.sc_morning_trail_buy_clothes) => {
