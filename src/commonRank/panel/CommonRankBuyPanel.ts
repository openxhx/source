namespace commonRank {
    export class BuyPanel extends ui.commonRank.panel.CommonRankBuyPanelUI {
        ...
        private updateTimeInfo() {
            let total1 = xls.get(xls.eventExchange).get(2796).limit.v2;
            let total2 = xls.get(xls.eventExchange).get(2797).limit.v2;
        ...
        private onExchange(idx: number) {
            ...
            if (idx == 2) {
                ...
                if (info.cost[0].v1 == clientCore.MoneyManager.SPIRIT_BEAN_MONEY_ID) {
                    alert.showSmall(`是否花费${info.cost[0].v2}灵豆购买10个十字架?`, {
                        callBack: {
                            caller: this, funArr: [() => {
