namespace newActivity {
     ...
     * 活动单元
     */
    export class ActivityItem extends ui.newActivity.item.ActivityItemUI implements util.IRedPoint {
        ...
        public setInfo(caller: NewActivityModule, value: xls.limitActivity) {
            this._cfg = value;
            ...
            if ((eDate.getTime() - sDate.getTime()) / (1000 * 3600 * 24) > 365) {
                this.timeTxt.changeText("常驻");
            } else {
                time = cfg.eventTimeShow.split('_');
                sDate = new Date(time[0].replace(/-/g, '/'));
                eDate = new Date(time[1].replace(/-/g, '/'));
                this.timeTxt.changeText(`${sDate.getMonth() + 1}/${sDate.getDate()}~${eDate.getMonth() + 1}/${eDate.getDate()}`);
            }
        ...
        private onClick(): void {
            //TODO 先屏蔽幸运勋章
            // if (this._cfg.id == 3){
            //     alert.showFWords('活动正在紧急维护中，请耐心等待游戏内公告');
            //     return;
            // }
            ...
            clientCore.ModuleManager.closeModuleByName('newActivity');
            if (module.name == "springFaerie.SpringFaerieModule") {
                clientCore.MedalManager.getMedal([MedalConst.SPRING_FAERIE_FIRST_3])
                    .then((msg: pb.ICommonData[]) => {
                        if (msg[0].value == 0) {
                            clientCore.ModuleManager.open("springFaerie.SpringFaerieGuideModule", module.extraData);
                        }else{
