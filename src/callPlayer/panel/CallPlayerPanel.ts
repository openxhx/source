namespace callPlayer {
    export class CallPlayerPanel extends ui.callPlayer.panel.CallPlayerPanelUI {
        private _model: CallPlayerModel;
        constructor(sign: number) {
            super();
            this.sideClose = true;
            this.labInput.restrict = "0-9";
            this._model = clientCore.CManager.getModel(sign) as CallPlayerModel;
        }
        ...
        /**提交密码 */
        private submitCode() {
            net.sendAndWait(new pb.cs_watch_and_pick_up_the_light_write_uid({ uid: Number(this.labInput.text) })).then(() => {
