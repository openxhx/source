namespace alert {
    export class BuyPanel extends ui.bigCharge.panel.BuyPanelUI {
        private eventId: number[];
        async show(ids: number[]): Promise<void> {
            this.eventId = ids;
            let timeArr: number[] = await clientCore.LimitRechargeManager.instance.getInfo();
            for (let i: number = 0; i < 2; i++) {
                let item: ui.bigCharge.render.BuyItemUI = this[`item${i}`];
        ...
        private async onClick(index: number, id: number): Promise<void> {
            ...
            let item: ui.bigCharge.render.BuyItemUI = this[`item${index}`];
            net.sendAndWait(new pb.cs_common_buy_times_check({ id: event.chargeId })).then((msg: pb.sc_common_buy_times_check) => {
                if(msg.flag){
                    clientCore.RechargeManager.pay(event.chargeId).then((data) => {
                        ...
                        let limit: xls.pair = event.limit;
                        if (event.limit.v1 != 3) item.btnBuy.disabled = ++value >= event.limit.v2;
                        ...
                        this.waiting = false;
                        EventManager.event("COIN_RERFESH");
                    }).catch(() => {
                        this.waiting = false;
                    });
                }else{
