namespace clientCore {
    ...
    export class ShareManager {
        private static _shareUI: ui.share.ShareHUI | ui.share.ShareVUI;
        ...
        static async showShare(direction: 'horizontal' | 'vertical'): Promise<boolean> {
            ...
            LayerManager.disableClickEffect();
            this._shareDirection = direction;
            //没有图集先加载图集
            if (!this._loadAtlas) {
                await ModuleManager.loadatlas('share');
                await ModuleManager.loadUnpack('share');
                this._loadAtlas = true;
            }
            this._shareUI = this._shareDirection == 'horizontal' ? new ui.share.ShareHUI() : new ui.share.ShareVUI();
            this._shareUI.x = LayerManager.OFFSET;
            this._shareUI.img.texture = Laya.stage.drawToCanvas(this.imgSize.width, this.imgSize.height, -LayerManager.OFFSET, 0).getTexture();
            //闪屏动效
        ...
        static async saveScreenShot(direction: 'horizontal' | 'vertical'): Promise<boolean> {
            ...
            //闪屏动效
            await this.flashScreen();
            let canvas = this.equalCanvas(Laya.stage.drawToCanvas(this.imgSize.width, this.imgSize.height, -LayerManager.OFFSET, 0));
            let base64 = canvas.toBase64('image/png', 0.9);
            if (!Laya.Render.isConchApp) {
                SaveFile.saveFile(`screenshot.png`, this.base64ToBlob(base64, "png"));
            }
            else {
                NativeMgr.instance.saveImage(base64, direction == 'horizontal' ? 'hengping' : 'shuping')
            }
        ...
        private static async flashScreen() {
            ...
            LayerManager.clickEffectLayer.addChild(this._flashSp);
        ...
        private static closeShare() {
            ...
            LayerManager.enableClickEffect();
        }
        ...
        private static goShare(platform: string) {
            ...
            if (!Laya.Render.isConchApp) {
                ...
                NativeMgr.instance.shareImage(base64, platform, this._shareDirection == 'horizontal' ? 'hengping' : 'shuping')
            }
        ...
        private static onShareOver(state: number) {
            ...
            if (!this._isReward) {
                net.sendAndWait(new pb.cs_get_share_award({ idx: 1 })).then((msg: pb.sc_get_share_award) => {
        ...
        private static updateReward() {
            this._shareUI.imgFirst.visible = !this._isReward;
        }
