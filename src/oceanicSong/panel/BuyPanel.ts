namespace oceanicSong {
    export class BuyPanel extends ui.oceanicSong.panel.BuyPanelUI {
        private eventId: number[];
        async show(type: number): Promise<void> {
            this.eventId = type == 1 ? [5,6] : [1,2];
            ...
            for (let i: number = 0; i < 2; i++) {
                ...
                item.nameTxt.changeText(`${cfg.cost}元${type == 1 ? '探险' : '羽毛'}礼包`);
                item.limitTxt.visible = limit.v1 != 3;
                if(limit.v1 != 3){
                    item.limitTxt.changeText(`${limit.v1 == 1 ? '今日' : '本周'}剩余：${limit.v2 - times}/${limit.v2}`);
                    item.btnBuy.disabled = times == limit.v2;
                }
                (limit.v1 == 3 || times != limit.v2) && BC.addEvent(this, item.btnBuy, Laya.Event.CLICK, this, this.onClick, [i, event.id]);
