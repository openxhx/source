namespace snowNightFestival {
    export class BuyDetailsPanel extends ui.snowNightFestival.panel.BuyDetailsPanelUI {
        ...
        showInfo(rechargeID: number) {
            ...
            if (rechargeID == 36 && lastFloorTime < curFloorTime) {
                this._rewardArr = this._rewardArr.concat(clientCore.LocalInfo.sex == 1 ? rechargeInfo.rewardFamaleSpe : rechargeInfo.rewardMaleSpe);
            }
            if (this._rewardArr.length <= 3) this.width = 450;
            else this.width = 450 + (this._rewardArr.length - 3) * 120;
        ...
        private onBuyClick(e: Laya.Event) {
            EventManager.event("NIGHTFES_GIFT_BUY", this._rechargeID);
