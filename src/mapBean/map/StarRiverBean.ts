namespace mapBean {
     ...
     * 
     */
    export class StarRiverBean implements core.IMapBean {
        ...
        private _mainUI: ui.starRiverBean.StarRiverBeanUI;

        async start() {
            await this.updateList();
            await Promise.all([
                clientCore.ModuleManager.loadatlas('starRiver')
            ]);
            if (!this._destroy) {
                this.init();
            }
        ...
        init() {
            this._mainUI = new ui.starRiverBean.StarRiverBeanUI();
            this._mainUI.pos(473, 665);
        ...
        private onPanelClick(e: Laya.Event) {
            if (e.target != this._mainUI.btnBless) {
                clientCore.ModuleManager.open('starLover.StarLoverShowModule');
            }
        }

        /**请求刷新列表 */
        private updateList() {
            let timeOk = (clientCore.ServerManager.curServerTime - StarRiverBean._lastRefreshTime) > 10;//两次请求时间大于10s（如果当前列表很短，一下就展示完了）
            if (timeOk) {
                let listLen = StarRiverBean._allList.length;
                let reqNum = Math.min(StarRiverBean._totalLen - listLen, 30);//一次最多再拉取50个
                return net.sendAndWait(new pb.cs_show_list({ start: listLen, end: listLen + reqNum })).then((data: pb.sc_show_list) => {
                    StarRiverBean._lastRefreshTime = clientCore.ServerManager.curServerTime;
                    StarRiverBean._allList = StarRiverBean._allList.concat(data.showInfos);
                    StarRiverBean._allList = _.uniqBy(StarRiverBean._allList, o => o.uid);
                    StarRiverBean._totalLen = Math.max(StarRiverBean._allList.length, data.maxLength);
                });
            }
        }

        private onBless() {
            net.sendAndWait(new pb.cs_star_river_show_congrats({ uid: this._currShowId })).then((data: pb.sc_star_river_show_congrats) => {
                if (!this._destroy) {
                    alert.showReward(data.items);
                    this._mainUI.btnBless.visible = false;
                }
                if (_.find(StarRiverBean._allList, o => o.uid == this._currShowId))
                    _.find(StarRiverBean._allList, o => o.uid == this._currShowId).flag = 1;
                else
        ...
        private async createNextShower() {
            this._mainUI.btnBless.visible = false;
            if (StarRiverBean._allList.length == 0) {
                ...
                let clothInfo = _.find(StarRiverBean._allList, o => o.uid == this._currShowId);
                if (clothInfo)
        ...
        private getNextInfo() {
            ...
            while (ranInfo.uid == this._currShowId || ranInfo.friendUid == this._currShowId) {
        ...
        private getOptionalArr() {
            //先选取未点赞的
            let unLikeArr = _.filter(StarRiverBean._allList, o => o.flag == 0);
            if (unLikeArr.length == 0) {
                console.log('未点赞的示过一遍了,请求刷新')
                this.updateList();
                return StarRiverBean._allList;
            }
            else {
