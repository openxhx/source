namespace mapBean {
     ...
     * 
     */
    export class NightEsidenceBean implements core.IMapBean {
        ...
        private _mainUI: ui.nightEsidenceBean.NightEsidenceBeanUI;

        async start() {
            await this.updateList();
            await Promise.all([
                clientCore.ModuleManager.loadatlas('nightEsidenceBean')
            ]);
            if (!this._destroy) {
                this.init();
            }
        ...
        init() {
            this._mainUI = new ui.nightEsidenceBean.NightEsidenceBeanUI();
        ...
        private onPanelClick(e: Laya.Event) {
            if (e.target != this._mainUI.btnBless) {
                clientCore.ModuleManager.open('nightEsidence.NightEsidenceShowModule');
            }
        }

        /**请求刷新列表 */
        private updateList() {
            let timeOk = (clientCore.ServerManager.curServerTime - NightEsidenceBean._lastRefreshTime) > 10;//两次请求时间大于10s（如果当前列表很短，一下就展示完了）
            if (timeOk) {
                let listLen = NightEsidenceBean._allList.length;
                let reqNum = Math.min(NightEsidenceBean._totalLen - listLen, 30);//一次最多再拉取50个
                return net.sendAndWait(new pb.cs_dark_night_hall_show_list({ start: listLen, end: listLen + reqNum })).then((data: pb.sc_dark_night_hall_show_list) => {
                    NightEsidenceBean._lastRefreshTime = clientCore.ServerManager.curServerTime;
                    NightEsidenceBean._allList = NightEsidenceBean._allList.concat(data.showInfos);
                    NightEsidenceBean._allList = _.uniqBy(NightEsidenceBean._allList, o => o.uid);
                    NightEsidenceBean._totalLen = Math.max(NightEsidenceBean._allList.length, data.maxLength);
                });
            }
        }

        private onBless() {
            net.sendAndWait(new pb.cs_dark_night_hall_show_congrats({ uid: this._currShowId })).then((data: pb.sc_dark_night_hall_show_congrats) => {
                if (!this._destroy) {
                    alert.showReward(data.items);
                    this._mainUI.btnBless.visible = false;
                }
                if (_.find(NightEsidenceBean._allList, o => o.uid == this._currShowId))
                    _.find(NightEsidenceBean._allList, o => o.uid == this._currShowId).flag = 1;
                else
        ...
        private async createNextShower() {
            this._mainUI.btnBless.visible = false;
            if (NightEsidenceBean._allList.length == 0) {
                ...
                let clothInfo = _.find(NightEsidenceBean._allList, o => o.uid == this._currShowId);
                if (clothInfo)
        ...
        private async createRole() {
            ...
            if (!this._destroy && clothInfo) {
                ...
                nick1.y = nick2.y = -130;
                nick1.color = nick2.color = '#CCFF33';
        ...
        private getOptionalArr() {
            //先选取未点赞的
            let unLikeArr = _.filter(NightEsidenceBean._allList, o => o.flag == 0);
            if (unLikeArr.length == 0) {
                console.log('未点赞的示过一遍了,请求刷新')
                this.updateList();
                return NightEsidenceBean._allList;
            }
            else {
