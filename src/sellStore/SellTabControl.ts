namespace sellStore {
    ...
    export class SellTabControl {
        ...
        private _childHash: util.HashMap<Laya.List>;//自己按钮list Hash
        ...
        public getClothListCell(pTab: number, cTab: number) {
            let list = this._childHash.get(pTab);
            let arr = list.dataSource;
            let index = arr.indexOf(cTab);
            return list.getCell(index);
            // this._childHash.get(clientCore.CLOTH_TYPE.CLOTH_ARR).getCell(2);
        }
        ...
        */
        ...
        private onParentRender(cell: ui.commonUI.item.ClothTabButtonUI, clothType: number) {
            ...
            if (clothType == TIME_LIMIT) {
                cell.imgBg.skin = 'clothTab/mainTab1.png';
                cell.imgArrow.skin = 'clothTab/xiala_specialtab.png';
            }
        ...
        private sureSelect(selectClothType: number) {
            if (this._selectType != selectClothType) {
                ...
                //触发回调
                this._hanlder.runWith(this._selectType);
            }
            if (clientCore.GuideMainManager.instance.curGuideInfo.operationBehavior == "clickSellStoreModuleTab") {
