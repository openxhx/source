namespace flowerMass {
     ...
     * 感恩回馈
     */
    export class FeedbackPanel extends ui.flowerMass.panel.FeedbackPanelUI {
        private readonly suitId: number = 2110620;
        ...
        show(box: any) {
            clientCore.Logger.sendLog('2021年3月25日活动', '【付费】小花仙集合吧', '打开周年回馈面板');
        ...
        private setUI() {
            this._engryCnt = FlowerMassModel.instance.costAllCnt;
            this.labCur.text = "" + this._engryCnt;
        ...
        private listRender(item: ui.springOverture.render.FeedbackItemUI) {
            ...
            item.imgGot.visible = util.getBit(FlowerMassModel.instance.feedbackRewardFlag, data.packageID) == 1;
        ...
        private getReward(idx: number) {
            net.sendAndWait(new pb.cs_common_feedback_get_reward({ idx: this.reward[idx].packageID, activityId: FlowerMassModel.instance.activityId })).then(async (msg: pb.sc_common_feedback_get_reward) => {
                alert.showReward(msg.items);
                FlowerMassModel.instance.feedbackRewardFlag = util.setBit(FlowerMassModel.instance.feedbackRewardFlag, this.reward[idx].packageID, 1);
                this.reward = this.bubbleSort(this.reward);
                this.list.refresh();
