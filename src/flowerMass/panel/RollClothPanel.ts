namespace flowerMass {
     ...
     * 转盘抽衣服
     */
    export class RollClothPanel extends ui.flowerMass.panel.RollClothPanelUI {
        /** 抽奖功能ID*/
        private _poolId: number = 39;
        /** 抽奖使用的道具ID*/
        private _costItemId: number = 9900332;
        /**套装ID */
        private _suitId: number = 2110649;
        /**集齐奖励ID */
        private _giftId1: number = 1200038;
        private _giftId2: number[] = [1000189, 1100132];
        /**帮助说明id */
        ...
        constructor() {
            ...
            this.imgFaceReward.skin = clientCore.LocalInfo.sex == 1 ? "flowerMass/RollClothPanel/nv_mei_tong.png" : "flowerMass/RollClothPanel/nan_mei_tong.png";
            this.addEvents();
            //this.btnOther.visible = false;
        }

        show(box: any) {
            clientCore.Logger.sendLog('2022年4月29日活动', '【付费】小花仙集合啦', '打开周年扭蛋-梦语岚珊面板');
            this.setShowInfo(0);
            box.addChild(this);
            ...
            if (curTime >= util.TimeUtil.formatTimeStrToSec("2022-5-6 00:00:00")) {
                this.btnCur.x = 299;
                this.btnOther.x = 66;
                this.tabName2.skin = "flowerMass/RollClothPanel/jie_qi_li_xia0.png"
            }
        }
        ...
        private initView(): void {
            ...
            //格子初始化
            this._array = _.filter(xls.get(xls.rouletteDraw).getValues(), (element: xls.rouletteDraw) => { return element.type == FlowerMassModel.instance.activityId && element.period == this._poolId; });
            this._cost = _.filter(xls.get(xls.rouletteDrawCost).getValues(), (element: xls.rouletteDrawCost) => { return element.type == FlowerMassModel.instance.activityId && element.period == this._poolId });
        ...
        private changePanel() {
            ...
            if (curTime < util.TimeUtil.formatTimeStrToSec("2022-5-6 00:00:00")) {
                ...
                EventManager.event(CHANGE_PANEL, subpanel.rollCloth2);
            }
        }
        ...
        private onTry(i: number): void {
            if (i == 0) {
                ...
                clientCore.ModuleManager.open('previewBG.PreviewBGModule', { id: this._giftId2, condition: '' });
            } else {
        ...
        private updateCurrent(): void {
            ...
            if (this.boxCur.visible) {
                this.imgCur.skin = `flowerMass/RollClothPanel/${this.curDiscount / 10}.png`;
            }
        }
        ...
        private updateGrid(cfg: xls.rouletteDraw, index: number): void {
            let item: ui.flowerMass.render.RollClothItemUI = this.boxItem.getChildAt(index) as ui.flowerMass.render.RollClothItemUI;
        ...
        private set select(value: number) {
            ...
            let item: ui.flowerMass.render.RollClothItemUI;
            if (this._select != -1) {
                item = this.boxItem.getChildAt(this._select % len) as ui.flowerMass.render.RollClothItemUI;
                item.imgSelect.visible = false;
            }
            this._select = select;
            item = this.boxItem.getChildAt(this._select % len) as ui.flowerMass.render.RollClothItemUI;
        ...
        private playDiscountAni(discount: number) {
            this.imgNext.skin = `flowerMass/RollClothPanel/next_${discount / 10}.png`;
            // return new Promise((suc: Function) => {
        ...
        private updateGift(): void {
            let hasBg: boolean = clientCore.ItemsInfo.checkHaveItem(this._giftId2[0]);
