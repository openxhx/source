namespace diningCarSell {
    ...
    export class DiningCarGameModule extends ui.diningCarSell.DiningCarSellModuleUI {
        ...
        init(data: any) {
            ...
            this.curLevel = data.gameId;
            this.addPreLoad(xls.load(xls.park));
            this.addPreLoad(xls.load(xls.gameDining));
        ...
        async onPreloadOver() {
            ...
            this.initFood();
            this.initView();
            let msg = await clientCore.MedalManager.getMedal([MedalConst.DINING_CAR_GAME_FIRST]);
            this.isFirst = msg[0].value == 0;
            if (this.isFirst) {
                clientCore.ModuleManager.open("diningCarRule.DiningCarRuleModule", 1);
                clientCore.MedalManager.setMedal([{ id: MedalConst.DINING_CAR_GAME_FIRST, value: 1 }]);
            } else {
                this.startGame();
            }
        ...
        private creatSellInfo() {
            let carConfig = xls.get(xls.gameDining).get(this.curLevel);
            ...
            this.customerCnt = maxCustomer;
            this.customerTime = waitTime;
            this.customerCdInfo = this.customerCd = cd;
            //机器信息
            let machines = [];
        ...
        private initView() {
            this.targetPoint = xls.get(xls.park).get(this.gameId).passType;
            this.labScore.text = "本次营业额：0/" + this.targetPoint;
        ...
        private onPause() {
            alert.showSmall('是否确认退出？退出后不计算当前进度，无法获得奖励。', { callBack: { caller: this, funArr: [this.sureClose, this.continue] } });
        }
        ...
        private async gameOver() {
            ...
            net.sendAndWait(new pb.cs_mini_game_collect_over({ id: this.gameId, score: this.curCoin })).then((msg: pb.sc_mini_game_collect_over) => {
                if (msg.items.length == 0) {
                    alert.showFWords('经营结束，本次未获得奖励~');
                    this.sureClose();
                    return;
                }
                alert.showReward(msg.items, '', { callBack: { caller: this, funArr: [this.sureClose] } });
            }).catch(() => {
                this.sureClose();
            });
        }
        ...
        private creatCustomer() {
            ...
            customer.needFood = this.getRandomFood();
            customer.coin = customer.needFood.length * 10;
            customer.waitTime = this.customerTime[customer.needFood.length - 1];
            customer.endTime = clientCore.ServerManager.curServerTime + customer.waitTime;
            customer.show();
            this.finishCnt += 1;
            this.creatCustomer();
        }
        ...
        private getRandomFood() {
            let target = [];
            let config = xls.get(xls.gameDining).get(this.curLevel).customerOrderNum;
            for (let i: number = 0; i < config.length; i++) {
                if (this.finishCnt < config[i].v1) {
                    if (config[i].v1 - this.finishCnt <= config[i].v3) {//两个食物
                        let idx = Math.floor(Math.random() * xls.get(xls.diningCarFood).length);
                        target.push(parseInt(xls.get(xls.diningCarFood).getKeys()[idx]));
                        idx = Math.floor(Math.random() * xls.get(xls.diningCarFood).length);
                        target.push(parseInt(xls.get(xls.diningCarFood).getKeys()[idx]));
                        break;
                    } else {
                        let idx = Math.floor(Math.random() * xls.get(xls.diningCarFood).length);
        ...
        private creatCustomers() {
            ...
            for (let i: number = 0; i < 3; i++) {
                let customer: Customer = new Customer(true);
        ...
        private getCoin(num: number, pos: number, npc: number) {
            ...
            this.curCoin += num;
            this.labScore.text = "本次营业额：" + this.curCoin + "/" + this.targetPoint;
            if (this.curCoin >= this.targetPoint) {
                this.gameOver();
            } else if (this.isFinish() && this.finishCnt >= this.customerCnt) {
                this.gameOver();
