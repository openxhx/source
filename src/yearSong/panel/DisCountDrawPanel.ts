namespace yearSong {
     ...
     * 白幽灵
     */
    export class DisCountDrawPanel extends ui.yearSong.panel.DisCountDrawPanelUI {

        private _rotating: boolean = false;
        private _suitId: number = 2110540;
        private _giftId: number = 1000128;
        constructor() {
            super();
        ...
        async show() {
            ...
            clientCore.Logger.sendLog('2021年11月26日活动', '【付费】岁月如歌', '打开珠落玉盘-雪香寻面板');
            await net.sendAndWait(new pb.cs_star_sakura({ activeId: 197, stage: 5 })).then((data: pb.sc_star_sakura) => {
        ...
        private sureBuy() {
            ...
            net.sendAndWait(new pb.cs_star_sakura_buy_clothes({ activeId: 197, stage: 5 })).then((data: pb.sc_star_sakura_buy_clothes) => {
        ...
        private startTurn() {
            this._rotating = true;
            net.sendAndWait(new pb.cs_refresh_star_sakura({ activeId: 197, stage: 5 })).then((data: pb.sc_refresh_star_sakura) => {
        ...
        /**打开复出直购 */
        private openOther() {
            EventManager.event('YearSong_SHOW_EVENT_PANEL', panelType.disCountDrawNew);
        }
