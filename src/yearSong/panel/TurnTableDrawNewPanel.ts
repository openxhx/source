namespace yearSong {
    export class TurnTableDrawNewPanel extends ui.yearSong.panel.TurnTableDrawNewPanelUI {
        /** 抽奖功能ID*/
        private _poolId: number = 22;
        /** 抽奖使用的道具ID*/
        private _costItemId: number = 9900275;
        /**套装ID */
        private _suitId: number = 2110541;
        /**集齐奖励ID */
        private _giftId: number = 3500088;
        /**充值礼包标志 */
        /**帮助说明id */
        ...
        constructor() {
            ...
            clientCore.Logger.sendLog('2021年11月26日活动', '【付费】岁月如歌', '打开莺声婉转-梦中之蝶面板');
        }
        ...
        private openOther() {
            EventManager.event('YearSong_SHOW_EVENT_PANEL', panelType.turnTableDraw);
        }
        ...
        private updateRewrd(): void {
            let arr:number[] = [140753 , 140764];
            let suitInfo = clientCore.SuitsInfo.getSuitInfo(this._suitId);
            this.boxEye.visible = !suitInfo.allGet;
            let temp = suitInfo.hasCnt;
            if (!suitInfo.allGet) {
                this.labProgress.changeText(`${temp}/${suitInfo.clothes.length - 3}`);
                this.boxEye.mouseEnabled = temp  >= suitInfo.clothes.length - 3;
            }
        }
        //#region 抽奖效果
