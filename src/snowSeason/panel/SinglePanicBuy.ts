namespace snowSeason {
    export class SinglePanicBuy extends ui.snowSeason.panel.SinglePanicBuyPanelUI {
        private readonly suitId: number = 2110566;
        ...
        private buyInfoRender(item: ui.snowSeason.render.PanicInfoRenderUI) {
        ...
        private offInfoRender(item: ui.snowSeason.render.LimitInfoRenderUI) {
            let off: number = item.dataSource;
            item.imgFlag.skin = (off >= this.curOff && this.curOff > 0) ? "snowSeason/SinglePanicBuyPanel/re_mai_zhong.png" : "snowSeason/SinglePanicBuyPanel/yi_shou_wan.png"
        ...
        async show() {
            ...
            clientCore.Logger.sendLog('2021年12月31日活动', '【付费】初雪的季节', '打开特卖会-鸿俦鹤侣面板');
        }
        ...
        private setUI() {
            ...
            this.imgOff.skin = `snowSeason/SinglePanicBuyPanel/dang_qian${this.curOff}_zhe.png`;
            this.listOffInfo.refresh();
        }

        /**检查开卖时间 */
        private checkSell(): number {
            let endT: number = util.TimeUtil.formatTimeStrToSec("2021/12/31 20:10:00");
        ...
        private buySuit() {
            ...
            if (have < cost) {
                alert.showSmall(`所需${clientCore.ItemsInfo.getItemName(SnowSeasonModel.instance.coinid)}不足,是否前往补充?`, { callBack: { funArr: [SnowSeasonModel.instance.coinNotEnough], caller: this } });
                return;
            }
            let off: number = this.curOff; //打开的时候 需要缓存一下折扣值 以防打开的时候折扣发生变化... 有争议
            alert.showSmall(`是否花费${cost}${clientCore.ItemsInfo.getItemName(SnowSeasonModel.instance.coinid)}购买鸿俦鹤侣套装?`, {
                callBack: {
                    caller: this, funArr: [async () => {
                        ...
                        if (res) {
                            SnowSeasonModel.instance.coinCost(cost);
