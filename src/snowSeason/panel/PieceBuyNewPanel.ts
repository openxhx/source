namespace snowSeason {
    export class PieceBuyNewPanel extends ui.snowSeason.panel.PieceBuyPanelNewUI {

        private buyState: number;
        private timeStamp: number;
        private suitId: number = 2110569;
        ...
        show() {
            this.addPreLoad(net.sendAndWait(new pb.cs_patch_buy_info({type:3})).then((msg: pb.sc_patch_buy_info) => {
            ...
            this.suitShow1.visible = clientCore.LocalInfo.sex == 1;
            this.suitShow2.visible = clientCore.LocalInfo.sex == 2;
            this.blockBg.skin = `unpack/snowSeason/blockNew${clientCore.LocalInfo.sex}.png`;
            for (let i: number = 0; i < 3; i++) {
                this["blockbg" + i].skin = clientCore.LocalInfo.sex == 1 ? `snowSeason/PieceBuyPanel/blockfemaleNew_${i}.png` : `snowSeason/PieceBuyPanel/blockmaleNew_${i}.png`;
            }
            clientCore.Logger.sendLog('2022年1月7日活动', '【付费】初雪的季节', '打开碎琼乱玉-莲衣铁羽面板');
        }
        ...
        private openOther() {
            EventManager.event('SnowSeason_SHOW_EVENT_PANEL', panelType.pieceBuy);
        }
                ...
                callBack: {
                    ...
                    funArr: [() => {
                        if (have < this.pieceCost) {
                            ...
                            net.sendAndWait(new pb.cs_patch_buy_reward({ index: i + 1  , type:3})).then((msg: pb.sc_patch_buy_reward) => {
                                alert.showFWords("碎片开启成功~");
                                if (msg.item && msg.item.length > 0) {
                                    alert.showReward(msg.item);
                                }
                                net.sendAndWait(new pb.cs_patch_buy_info({type:3})).then((msg: pb.sc_patch_buy_info) => {
                                    this.buyState = msg.buyBit == null ? 0 : msg.buyBit;
                                    this.timeStamp = msg.time == null ? 0 : msg.time;
                                    this.initPanel();
                                })
                ...
                callBack: {
                    ...
                    funArr: [() => {
                        if (have < this.suitCost) {
                            ...
                            net.sendAndWait(new pb.cs_patch_buy_reward({ index: 4 , type:3})).then((msg: pb.sc_patch_buy_reward) => {
                                alert.showReward(msg.item);
                                net.sendAndWait(new pb.cs_patch_buy_info({type:3})).then((msg: pb.sc_patch_buy_info) => {
                                    this.buyState = msg.buyBit == null ? 0 : msg.buyBit;
                                    this.timeStamp = msg.time == null ? 0 : msg.time;
