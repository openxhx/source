namespace greatNavigation {
    export class NavigationBuyPanel extends ui.greatNavigation.panel.NavigationBuyPanelUI {
        ...
        private updateTimeInfo() {
            let total1 = xls.get(xls.eventExchange).get(2675).limit.v2;
            let total2 = xls.get(xls.eventExchange).get(2676).limit.v2;
        ...
        private onExchange(idx: number) {
            let info = _.filter(xls.get(xls.eventExchange).getValues(), o => o.type == 160)[idx];
            ...
            if (info.cost[0].v1 == clientCore.MoneyManager.SPIRIT_BEAN_MONEY_ID) {
                alert.showSmall(`是否花费${info.cost[0].v2}灵豆购买10个星锚?`, {
                    callBack: {
                        caller: this, funArr: [() => {
        ...
        private sureExchange(id: number, idx: number) {
            net.sendAndWait(new pb.cs_common_exchange({ activityId: 160, exchangeId: id })).then((data: pb.sc_common_exchange) => {
