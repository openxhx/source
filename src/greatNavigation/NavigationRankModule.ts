namespace greatNavigation {
    ...
    /**
     * 大航海时代排行榜
     * greatNavigation.NavigationRankModule
     */
    export class NavigationRankModule extends ui.greatNavigation.NavigationRankModuleUI {
        ...
        private onListRender(cell: ui.greatNavigation.renders.NavigationRankItemUI, idx: number) {
            if (this._closed)
                return;
            ...
            if (data && rank) {
                ...
                if (rank.ranking <= 3) {
                    ...
                    cell.imgTop.skin = `greatNavigation/top${rank.ranking}.png`;
                }
                else {
        ...
        private onRankRwdChange(idx: number) {
            for (let i = 0; i < 3; i++) {
                this['imgTop_' + i].skin = idx == i ? 'greatNavigation/xuan_zhong_zhuang_tai.png' : 'greatNavigation/wei_xuan_zhong_zhuang_tai.png';
                this['boxRwd_' + i].visible = idx == i;
            }
        }

        private onTry() {
            clientCore.ModuleManager.open('previewBG.PreviewBGModule', { id: 1200020, condition: '前十名可获得{乐园夜坐骑}', limit: '' });
        }
