namespace springOverture {
     ...
     * 千里莺啼
     */
    export class PanicBuyPanel extends ui.springOverture.panel.PanicBuyPanelUI {
        private coinId: number = 9900284;
        private readonly suitId: number = 2110606;
        private readonly price: number = 650;
        ...
        private offInfoRender(item: ui.springOverture.render.LimitInfoRenderUI) {
            let off: number = item.dataSource;
            item.imgFlag.skin = (off >= this.curOff && this.curOff > 0) ? "springOverture/PanicBuyPanel/re_mai_zhong.png" : "springOverture/PanicBuyPanel/yi_shou_wan.png";
        ...
        async show(box:any) {
            ...
            box.addChild(this);
            clientCore.Logger.sendLog('2022年3月4日活动', '【付费】春日序曲', '打开千里莺啼面板');
        }
        ...
        private setUI() {
            ...
            this.imgOff.skin = `springOverture/PanicBuyPanel/dang_qian${this.curOff}_zhe.png`;
            this.listOffInfo.refresh();
        }

        /**检查开卖时间 */
        private checkSell(): number {
            let endT: number = util.TimeUtil.formatTimeStrToSec("2022/3/4 20:10:00");
        ...
        private buySuit() {
            ...
            if (have < cost) {
                alert.showSmall(`所需${clientCore.ItemsInfo.getItemName(this.coinId)}不足,是否前往补充?`, { callBack: { funArr: [() => { SpringOvertureModel.instance.openCoinGiftBuy() }], caller: this } });
                return;
            }
