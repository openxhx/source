namespace springOverture {
    ...
    export class MainSellPanel {
        ...
        private async initUI() {
            this.mainUI.imgSuit1.skin = `unpack/springOverture/${this.cfg.suits[0]}_${clientCore.LocalInfo.sex}.png`;
            this.mainUI.imgSuit2.skin = `unpack/springOverture/${this.cfg.suits[1]}_${clientCore.LocalInfo.sex}.png`;
            if (this.cfg.phase == 1) {
                let madel = await clientCore.MedalManager.getMedal([this.cfg.offMedal]);
                this.mainUI.imgTip.visible = madel[0].value == 0;
            } else {
                let madel = await clientCore.MedalManager.getMedal([this.cfg.offMedal, MedalConst.MAIN_SELL_GET_FAERY]);
                this.mainUI.imgTip.visible = madel[0].value == 0;
                this.gotFaery = madel[1].value == 1;
            }
            this.checkGift();
        }
        ...
        show(box: any) {
            ...
            if (this.cfg.phase == 1) {
                clientCore.Logger.sendLog('2022年1月28日活动', '【付费】春日序曲第三期', '打开本周主打-吾爱永恒面板');
                this.getFaeryNumber();
        ...
        private getFaeryNumber() {
            net.sendAndWait(new pb.cs_get_jackpot_draw_item_info({ itemId: 2300032 })).then((msg: pb.sc_get_jackpot_draw_item_info) => {
        ...
        /**领取赠品 */
        private getGift() {
            net.sendAndWait(new pb.cs_common_recharge_get_ext_reward({ stage: this.cfg.giftToken[0], activityId: SpringOvertureModel.instance.activityId, index: this.cfg.giftToken[1] })).then((msg: pb.sc_common_recharge_get_ext_reward) => {
