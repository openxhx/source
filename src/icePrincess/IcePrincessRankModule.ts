namespace icePrincess {
    ...
    /**
     * 冰雪之殿的公主排行榜
     * icePrincess.IcePrincessRankModule
     */
    export class IcePrincessRankModule extends ui.icePrincess.IcePrincessRankModuleUI {
        ...
        private onListRender(cell: ui.icePrincess.renders.IceRankItemUI, idx: number) {
            if (this._closed)
                return;
            ...
            if (data && rank) {
                ...
                if (rank.ranking <= 3) {
                    ...
                    cell.imgTop.skin = `icePrincess/top${rank.ranking}.png`;
                }
                else {
        ...
        private onRankRwdChange(idx: number) {
            for (let i = 0; i < 3; i++) {
                this['imgTop_' + i].skin = idx == i ? 'icePrincess/xuan_zhong_zhuang_tai.png' : 'icePrincess/wei_xuan_zhong_zhuang_tai.png';
                this['boxRwd_' + i].visible = idx == i;
            }
        }

        private onTry() {
            clientCore.ModuleManager.open('previewBG.PreviewBGModule', { id: 1200007, condition: '前十名可获得{冰封王座坐骑}', limit: '' });
        }
