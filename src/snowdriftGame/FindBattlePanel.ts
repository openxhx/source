namespace snowdriftGame {
    /**
     * snowdriftGame.FindBattlePanel
     */
    export class FindBattlePanel extends ui.snowdriftGame.FindBattlePanelUI {
        ...
        init() {
            ...
            this.addPreLoad(net.sendAndWait(new pb.cs_snowman_max_score()).then((msg: pb.sc_snowman_max_score) => {
                this.max = msg.score;
            }));
        }
        ...
        popupOver() {
            ...
            net.sendAndWait(new pb.cs_snowman_match_user({ flag: 1 })).then(() => {
        ...
        private async loadAni() {
            await res.load("res/animate/snowGame/huabaoSnow.png");
            await res.load("unpack/happinessFlavour/MysticSnowdriftPanel/huabaoSnow.png");
        ...
        private async getBattle(msg: pb.sc_draw_match_user_notify) {
            ...
            this.destroy();
            clientCore.ModuleManager.open("snowdriftGame.SnowdriftGameModule", { other: other, max: this.max,room:msg.room });
        }
        ...
        destroy() {
            ...
            if (this.labSearching.visible) {
                net.send(new pb.cs_snowman_match_user({ flag: 0 }));
            }
            super.destroy();
        }
    }
