namespace halloweenShop {
    ...
    const SUIT_ID = 2110512;
    /**
     * 白幽灵
     */
    export class DisCountDrawPanel extends ui.halloweenShop.panel.DisCountDrawPanelUI {

        private _rotating: boolean = false;
        private giftId1:number = 1100107;
        private giftId2:number = 1000152;
        constructor() {
            super();
        ...
        async show() {
            ...
            clientCore.Logger.sendLog('2021年10月29日活动', '【付费】搞怪糖果商城', '打开南瓜约定面板');
            await net.sendAndWait(new pb.cs_star_sakura({ activeId: 202  , stage :1})).then((data: pb.sc_star_sakura) => {
        ...
        private updateView() {
            ...
            for (let i = 0; i < 5; i++) {
                this['zhe_' + i].skin = i == discountIdx ? `halloweenShop/DisCountDrawPanel/${DISCOUNT_STRING[i]}1.png` : `halloweenShop/DisCountDrawPanel/${DISCOUNT_STRING[i]}0.png`;
            }
        ...
        private sureBuy() {
            ...
            net.sendAndWait(new pb.cs_star_sakura_buy_clothes({activeId:202 , stage :1})).then((data: pb.sc_star_sakura_buy_clothes) => {
        ...
        private onSuit(i:number) {
            if(i==0){
                ...
                clientCore.ModuleManager.open('previewBG.PreviewBGModule', {id: [this.giftId1 , this.giftId2 ], condition: '', limit: ''});
            }
        }
        ...
        private startTurn() {
            this._rotating = true;
            net.sendAndWait(new pb.cs_refresh_star_sakura({activeId:202 , stage :1})).then((data: pb.sc_refresh_star_sakura) => {
        ...
        private async showTurnAni(start: number, target: number, time: number) {
            while (true) {
                ...
                this['zhe_' + start].skin = `halloweenShop/DisCountDrawPanel/${DISCOUNT_STRING[start]}0.png`;
                start++;
                start = start % 5;
                this['zhe_' + start].skin = `halloweenShop/DisCountDrawPanel/${DISCOUNT_STRING[start]}1.png`;
        ...
        private changePanel(){
            EventManager.event('HalloweenShop_SHOW_EVENT_PANEL', panelType.disCountDrawNew);
        }
