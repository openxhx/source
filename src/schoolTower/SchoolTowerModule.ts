namespace schoolTower {
    export class SchoolTowerModule extends ui.schoolTower.SchoolTowerModuleUI {
        private _model: SchoolTowerModel;
        private _control: SchoolTowerControl;

        private isFirst: boolean = false;
        /**帮助说明面板 */
        // private _helpPanel: SchoolTowerDetailPanel;
        /**超值礼包购买面板 */
        private _cheapBuyPanel: CheapBuyPanel;
        init(data?: any) {
            super.init(data);
            this.sign = clientCore.CManager.regSign(new SchoolTowerModel(), new SchoolTowerControl());
            this._control = clientCore.CManager.getControl(this.sign) as SchoolTowerControl;
            this._model = clientCore.CManager.getModel(this.sign) as SchoolTowerModel;
        ...
        async onPreloadOver() {
            ...
            clientCore.Logger.sendLog('2020年8月28日活动', '【付费】花蕾亚学园', '打开活动面板');
            this.setUI(true);
            let data: pb.ICommonData[] = await clientCore.MedalManager.getMedal([MedalConst.SCHOOL_TOWE_OPEN]);
            this.isFirst = data[0].value == 0 || data[0].changeTime < clientCore.ServerManager.getWeekUpdataSec();
        }

        popupOver() {
            if (this._model._gotReward.length <= 0 && this.isFirst) {
                this.openNewLevel();
                clientCore.MedalManager.setMedal([{ id: MedalConst.SCHOOL_TOWE_OPEN, value: 1 }])
            } else {
                this.showTargetLevel(1);
            }
        }
        ...
        private setUI(init: boolean = false) {
            ...
            for (let i = 1; i <= this._model.levelCount[this._model._curOpenLevel]; i++) {
                ...
                if (reward && this["window_" + i].img_window.visible) {
                    ...
                    if (!init) {
                        ...
                        let animate = clientCore.BoneMgr.ins.play("res/animate/schoolTower/Glass.sk", "open", false, this["window_" + i]);
                        animate.pos(54, 54);
            ...
            for (let i = 0; i < this._model.ticketIds.length; i++) {
                if (clientCore.ItemsInfo.getItemNum(this._model.ticketIds[i]) > 0) {
                    this.img_offeff.skin = `schoolTower/img_off${i}.png`;
            ...
            let sex = clientCore.LocalInfo.sex;
            let haveEye = clientCore.LocalInfo.checkHaveCloth(sex == 1 ? 4100668 : 4100671);
            this.img_extReward.skin = !haveEye ? `schoolTower/eye_${sex}.png` : 'schoolTower/img_gift.png';
            //获得了美瞳，没有买齐衣服不显示背景秀领取按钮
            if (haveEye) {
                this.btn_extReward.disabled = haveNum < clothIds.length;
                let haveBgShow = clientCore.BgShowManager.instance.checkHaveBgShow(1000037);
                if (haveBgShow) {
            ...
            this.btn_extReward.skin = this.btn_extReward.disabled ? "schoolTower/img_zi.png" : "schoolTower/getSuitRwd.png";
        ...
        private checkBuyCheapReward() {
            ...
            for (let i = 0; i < this._rechargeIDArr.length; i++) {
                let canBuy = clientCore.RechargeManager.checkBuyLimitInfo(this._rechargeIDArr[i]).lastTime < util.TimeUtil.floorTime(clientCore.ServerManager.curServerTime);
                if (canBuy) {
                    if (i == 0) {
                        this.imgRewardBox.skin = "schoolTower/img_6.png";
                    }
                    else if (i == 1) {
                        this.imgRewardBox.skin = "schoolTower/img_68.png";
                    }
        ...
        //#region 有新的层级可以开启
        /**开启新的层级 */
        private async openNewLevel() {
            this.mouseEnabled = false;
            await this.showTargetLevel(this._model._curOpenLevel);
            ...
            for (let i: number = this._model.levelCount[this._model._curOpenLevel]; i > this._model.levelCount[this._model._curOpenLevel - 1]; i--) {
                ...
                let img = new ui.schoolTower.render.RewardItemUI();
                ...
                Laya.Tween.to(img, { x: target.x, y: target.y }, 500, null, Laya.Handler.create(this, (img, i) => {
                    ...
                    this["window_" + i].img_window.visible = false;
                    img.destroy();
                    let animate = clientCore.BoneMgr.ins.play("res/animate/schoolTower/Glass.sk", "close", false, this["window_" + i]);
                    animate.pos(54, 54);
        ...
        /**将要显示的层级显示出来 */
        private async showTargetLevel(level: number) {
            let target: number = -1;
            if (level >= 6 && this.panel.vScrollBar.value > (7 - level) * 100) {
                target = (7 - level) * 100;
            }
            if (level <= 2 && this.panel.vScrollBar.value < 55 + (2 - level) * 100) {
                target = 55 + (2 - level) * 100;
            }
            if (target >= 0) {
                Laya.Tween.to(this.panel.vScrollBar, { value: target }, 200);
        ...
        private showLevelInfo(level: number) {
            ...
            let target = this["btn_info_" + level].localToGlobal(new Laya.Point(0, 29));
            target = this.globalToLocal(target);
            this.box_info.pos(target.x - this.box_info.width * 0.8, target.y - this.box_info.height / 2);
        ...
        private onWindowClick(pos: number) {
            ...
            if (this.img_off.visible) {
                if (this.img_offeff.skin == `schoolTower/img_off0.png`) cost *= 0.7;
                else if (this.img_offeff.skin == `schoolTower/img_off1.png`) cost *= 0.5;
                else if (this.img_offeff.skin == `schoolTower/img_off2.png`) cost *= 0.3;
            }
        ...
        private getExtReward() {
            let index = this.img_extReward.skin == 'schoolTower/img_gift.png' ? 2 : 1;
            //请求额外奖励
        ...
        private showExtReward() {
            ...
            clientCore.ModuleManager.open('previewBG.PreviewBGModule', { id: [1000037], condition: '集齐{鹅毛笔之旅套装}可获得', limit: '' });
        }
        ...
        //打开帮助面板
        private onHelp() {
            // this._helpPanel = this._helpPanel || new SchoolTowerDetailPanel();
            // this._helpPanel.show();
            alert.showRuleByID(1056);
        }
